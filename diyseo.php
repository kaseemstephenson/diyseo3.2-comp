...
function diyseo_output_meta_tags() {
    ...
    $has_meta = ($meta_title !== '' || $meta_description !== '');
    if (!$has_meta) {
        return;
    }
    ...
    $should_output = apply_filters('diyseo_should_output_meta', true, $post_id, $context);
...
function diyseo_plugin_row_meta($links, $file) {
    if (plugin_basename(__FILE__) === $file) {
        $new_links = array(
            'visit-site' => '<a href="https://diyseo.ai" target="_blank">' . __('Visit plugin site', 'diyseo') . '</a>'
        );
...
function diyseo_meta_title_box_callback($post) {
    ...
    ">
        ...
        <p style="margin-top: 16px; color: #93c5fd; font-size: 12px;">
            For best results, deactivate other SEO plugins to prevent duplicate metadata.
        </p>
...
function diyseo_register_block_patterns() {
    if (function_exists('register_block_pattern')) {
        register_block_pattern(
            ...
            array(
                'title' => __('DIY SEO Article Pattern', 'diyseo'),
                'content' => '<!-- wp:heading --><h2>Introduction</h2><!-- /wp:heading --><!-- wp:paragraph --><p>Introduction content</p><!-- /wp:paragraph -->',
                'categories' => array('text'),
                'description' => __('Standard article pattern with headings and paragraphs', 'diyseo'),
            )
        );
    }
}
